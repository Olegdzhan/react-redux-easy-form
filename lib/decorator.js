  ...
  class EasyFormHOC extends PureComponent {
    ...
    valueChanger = fieldKey => event => {
      ...
      if (typeof theField.validator === 'function' && theField.validateOnChange) {
        let errorMessage;
        const validationResult = theField.validator(value);
        if (typeof validationResult === 'number' && Array.isArray(theField.errorMessage)) {
          errorMessage = theField.errorMessage[validationResult]
        } else if (validationResult) {
          errorMessage = null;
        } else {
          errorMessage = theField.errorMessage;
        }
        validateField(formName, fieldKey, errorMessage);
      }
    };
    ...
    validateByKey = fieldKey => () => {
      const theField = fields[fieldKey];
      let errorMessage;
      const validationResult = theField.validator(this.getters[fieldKey]());
      if (typeof validationResult === 'number' && Array.isArray(theField.errorMessage)) {
        errorMessage = theField.errorMessage[validationResult]
      } else if (validationResult) {
        errorMessage = null;
      ...
      typeof theField.validator === 'function' && this.props.validateField(formName, fieldKey, errorMessage);
    };
    ...
    validate = ({ exclude } = {}) => {
      ...
      for (const key in filteredFields) {
        ...
        const validationResult = theField.validator(theValue);
        if (typeof theField.validator === 'function' && (typeof validationResult === 'number' || !validationResult)) {
          let errorMessage;
          if (typeof validationResult === 'number' && Array.isArray(theField.errorMessage)) {
            errorMessage = theField.errorMessage[validationResult];
          } else {
          ...
          validateField(formName, key, errorMessage);
          result = false;
        }
      }
