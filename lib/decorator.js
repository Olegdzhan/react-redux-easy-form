  ...
  class EasyFormHOC extends PureComponent {
    ...
    valueChanger = fieldKey => event => {
      ...
      if (typeof theField.validator === 'function' && theField.validateOnChange) {
        ...
        if (typeof validationResult === 'number') {
          errorMessage = Array.isArray(theField.errorMessage)
            ? (theField.errorMessage[validationResult] || theField.errorMessage[0])
            : theField.errorMessage;
    ...
    validate = ({ exclude } = {}) => {
      ...
      for (const key in filteredFields) {
        ...
        if (typeof theField.validator === 'function' && !theField.validator(theValue)) {
