...
export default ({
  ...
  class EasyFormHOC extends PureComponent {
    ...
    valueChanger = fieldKey => event => {
      ...
      console.warn(
        'RREF::ValidateOnChange',
        '\nformName:', formName,
        '\nfieldKey:', fieldKey,
        '\nvalidator is function:', typeof theField.validator === 'function',
        '\nvalue for validation:', value,
        '\nvalidation result:', theField.validator && theField.validator(value),
      );
    ...
    validate = ({ exclude } = {}) => {
      ...
      for (const key in filteredFields) {
        ...
        console.warn(
          ...
          '\nfieldKey:', key,
          '\nvalidator is function:', typeof theField.validator === 'function',
          '\nvalue for validation:', theValue,
          '\nvalidation result:', theField.validator && theField.validator(theValue),
        );
        if (typeof theField.validator === 'function') {
          validateField(formName, key, theField.validator(theValue));
          result = false;
        }
      }
