...
export default (formConfig: TEasyFormConfig) => (ReactComponent: Function) => {
  const {
    formName,
    fields,
    formValidator,
  ...
  class EasyFormHOC extends PureComponent<TEasyFormHocProps> implements ISetMethodsOverFieldsResult {
    ...
    setHelpers(methodsOverFieldsResult: ISetMethodsOverFieldsResult): void {
      Object.keys(methodsOverFieldsResult).forEach((key: $Keys<ISetMethodsOverFieldsResult>): void => {
        // $FlowFixMe
    ...
    validate = ({ exclude }: TEasyFormValidateConfig = {}): boolean => {
      ...
      let result = true;
      const filteredFields: { [k: string]: TEasyFormFieldConfig } = { ...fields };
      if (exclude?.length) {
